...
public final class Users implements CrudHandler {
  ...
  public void getOne(
    ...
    context.status(HttpStatus.OK_200).json(user);
  }

  private User findUserOrThrow(@NotNull String userId) {
    final List<User> users = repository.find(eq("userId", userId)).toList();
    throwNotFoundIfEmpty(userId, users);
    return users.get(0);
  }
  ...
  public void delete(
    ...
    repository.remove(findUserOrThrow(userId));
    context.status(HttpStatus.OK_200);
  }

  private void throwNotFoundIfEmpty(@NotNull String userId,
      List<User> users) {
    if (users.isEmpty()) {
